...
→ 계정 홈(/my-stores) 이동
→ 가게 추가하기 → /store-selection
→ 새 가게 등록 → 초기 세팅 위저드(/initial-setup) 진입
...
└─ shared/
   ...
   ├─ constants/
   └─ types/
   (Zustand 스토어는 features/*/store/ 각 도메인 내에 위치)
...
| 문제 상황                  | 원인                              | 해결 방법                                     |
| -------------------------- | --------------------------------- | --------------------------------------------- |
| 로그인 후 화면이 안 넘어감 | 카카오 OAuth 콜백 처리 지연       | 브라우저 새로고침 후 `/my-stores`로 직접 이동 |
